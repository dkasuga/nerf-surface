...
def plot(model, level, indices, model_outputs, pose, rgb_gt, path, epoch, img_res, plot_nimgs, max_depth, resolution):
    ...
    surface_traces = get_surface_trace(path=path,
                                       epoch=epoch,
                                       sdf=lambda x: model.sdf_network(x)[:, 0],
                                       level=level,
                                       resolution=resolution
                                       )
    print("surface_traces:{}".format(surface_traces))
...
def get_surface_trace(path, epoch, sdf, level=0.0, resolution=100, return_mesh=False):
    ...
    print("np.min(z):{}".format(np.min(z)))
    print("np.max(z):{}".format(np.max(z)))
